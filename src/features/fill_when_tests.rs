...
#[test]
fn diagnostics_when_inside_lambda_body_last_expr() {
    // `when` as last expression in a lambda body is expression-form and must be exhaustive.
    // See: https://github.com/Hessesian/kotlin-lsp/issues (regression: statement-form guard)
    let sealed_src = "sealed class Effect { object Success : Effect(); object Failure : Effect() }";
    let main_src = "\
fun test(e: Effect) {
    run {
        when (e) {
            is Effect.Success -> \"ok\"
        }
    }
}
";
    let idx = setup(&[("/Effect.kt", sealed_src), ("/main.kt", main_src)]);
    let u = uri("/main.kt");
    let diags = when_diagnostics(&idx, &u);
    assert_eq!(
        diags.len(),
        1,
        "expected 1 diagnostic for missing Failure branch"
    );
    assert!(
        diags[0].message.contains("Failure"),
        "message: {}",
        diags[0].message
    );
}
...
#[test]
fn diagnostics_when_statement_form_no_false_positive() {
    // `when` that is NOT the last expr in a lambda should not produce diagnostics.
    let sealed_src = "sealed class Effect { object Success : Effect(); object Failure : Effect() }";
    let main_src = "\
fun test(e: Effect) {
    run {
        when (e) {
            is Effect.Success -> println(\"ok\")
        }
        println(\"done\")
    }
}
";
    ...
    assert!(
        diags.is_empty(),
        "statement-form when must not produce diagnostics, got: {diags:?}"
    );
}
