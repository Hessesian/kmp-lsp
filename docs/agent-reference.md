# kmp-lsp — Agent Reference

> **On-demand reference material.** Load this when you need deep context — architecture, coding guidelines with examples, test conventions, patterns. The lean behavioral rules live in `AGENTS.md` (injected every turn).

## Project overview

`kmp-lsp` is a lightweight LSP server for Kotlin and Java files, built in Rust using `tower-lsp` and `tree-sitter`. It is designed for **agentic use** (Copilot CLI, Neovim, Helix) where fast symbol lookup matters more than compiler accuracy.
...
- **Workspace root override** — reads `~/.config/kmp-lsp/workspace` at startup

## Build & install

```sh
cargo build --release
cargo install --path .    # installs to ~/.cargo/bin/kmp-lsp
```
...
1. `KMP_LSP_WORKSPACE_ROOT` env var (if set and is a valid dir)
2. Client `rootUri` / first `workspaceFolder` (or `~/.config/kmp-lsp/workspace` in Copilot CLI mode with `KMP_LSP_PREFER_CONFIG_ROOT` set)
3. `~/.config/kmp-lsp/workspace` plain-text file (trimmed)

To switch workspace at runtime:
```sh
echo /path/to/project > ~/.config/kmp-lsp/workspace
# then kill & restart the LSP process
```

## Custom LSP commands

Registered in `execute_command_provider` capabilities:

| Command | Args | Effect |
|---|---|---|
| `kmp-lsp/reindex` | none | Clear cache, re-scan all files |
| `kmp-lsp/clearCache` | none | Clear in-memory index + disk cache |
...
Cache stored in `~/.cache/kmp-lsp/<root-hash>/index.bin` (bincode format).
...
- `"sourcePaths"` — library source dirs (excluded from references/rename). An explicit `[]` overrides the global `~/.kmp-lsp/sources` default.
- `<WORKSPACE>` placeholder is substituted with the absolute workspace root path.
- Write `{"sourcePaths":[]}` in tests to prevent scanning `~/.kmp-lsp/sources` or the Android SDK.
...
cherry-picked for relevance to kmp-lsp's architecture.
...
Apply to kmp-lsp: `WorkspaceActor` is `GlobalState`. Read-path handlers (`hover`, `definition`,
...
One `crossbeam::select!` dispatches all. Apply to kmp-lsp: `WorkspaceEvent` already follows
...
Apply to kmp-lsp: when handling `WorkspaceEvent::FileChanged`, drain remaining `FileChanged`
...
transitions `false → true`. Apply to kmp-lsp: `WorkspacePhase::is_ready()` is the simpler
...
Apply to kmp-lsp: `WorkspaceActor::handle_file_changed` already uses
