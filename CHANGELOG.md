...
- **Find-references now discovers usages reached only through inferred receiver types** — `val response = repository.openBody(); response.isOnline` was invisible to `textDocument/references` when nothing in that file named or imported `Body` directly (the type is known only through `openBody()`'s declared return type). This is common, idiomatic Kotlin — dependency-injected repositories, factory patterns — and previously made find-references on a field/method return silently incomplete results with no indication anything was missed. Discovery now also follows a bounded "producer hop": from files that do mention the declaring class, find members that return it, then find callers of those members too. Applies to fields, methods nested inside another class, and interface methods. A Kotlin `data class`'s compiler-synthesized `copy()` (and an `enum class`'s `values`/`valueOf`/`entries`) — which return the owning type for literally every data/enum class in a workspace — are excluded from this as producer signals; left in, `copy` alone matched over a thousand files in a real ~18k-file Android monorepo, enough on its own to blow the discovery budget and silently disable the feature for every data-class field. Verified end-to-end against a real reported case: 0 → 8 real usages found across 6 files, none of which imported the declaring class.
- **`R.jar` — each Android module's own generated resource class is now indexed** — `R.string`, `R.drawable`, `R.id`, `R.layout`, … were never resolvable before, since `R.jar` is produced by AAPT at build time and is never a Gradle dependency the existing JAR discovery would find. Member-reference recall +1.6pp on a real corpus measurement.
- **`kmp-lsp refs` gains `-C`/`--context <N>`** — inlines `N` lines of source around each match (in both text and `--json` output), cutting down on follow-up file-view round trips for agent/CLI callers.
- **JetBrains-Darcula theme for Neovim and Helix** — ports the existing Helix Darcula theme (tree-sitter highlight overrides included) to Neovim, and publishes it for Helix directly.

### Bug fixes

Cumulative effect of this cycle's resolver fixes, measured on a real ~13k-file Android monorepo: member-reference recall rose from 85.8% to 91.6%.

- **Compiler-intrinsic Kotlin members were unresolvable** — the 8 primitive scalar types (`Int`, `Long`, `Double`, `Float`, `Boolean`, `Byte`, `Short`, `Char`) have no compiled `.class` file of their own; `Int.MAX_VALUE`, `Char.isDigit(...)`, and similar companion-member access now fall back to the real `java.lang` box type. `enum class`'s compiler-generated `entries`/`values()`/`valueOf()` are now synthesized at parse time the same way, since they never appear as literal source text either.
- **JAR-compiled enum constants and named nested types were invisible to the indexer** — `BufferOverflow.DROP_OLDEST`-style enum constants inside a compiled JAR were never read; a JAR class's real named nested types (not just its companion object) were excluded by an over-broad filter, making constructs like `Moshi.Builder(...)` completely unresolvable.
- **A JAR type used only via inference, never explicitly imported, resolved to nothing** — full-IO go-to-definition/hover now reaches the same global-definitions-index fallback the lighter-weight IndexOnly resolution path already had; paired with a new same-package import-evidence tie-break for ambiguous bare names. The single largest recall improvement of this cycle: bare-reference recall +5.9pp on a real corpus measurement.
- **`R.jar` variant selection was non-deterministic** across multiple "debug"-shaped Android build variants — previously depended on unspecified filesystem directory-listing order.
- **Hover now declines rather than guessing on an ambiguous qualified reference** — it shared go-to-definition/rename's overload-collision bug (see below) but through its own separate resolution path, so fixing the first didn't fix hover.
- **Qualified member lookups silently picked the wrong overload** — collapsing to a single `.find()` result (rather than returning every overload) meant Java methods landed on whichever overload happened to come last in source order; fixed across the member resolver, then swept to the extension-function resolver, which had the identical bug and hadn't been covered by the first fix.
- **A same-named JAR/library decoy could shadow the real symbol** — ambiguous bare-name resolution now prefers a candidate in one of Kotlin's own default-import packages before falling back to weaker heuristics (`apply`/`run` colliding with unrelated JVM members was the most common trigger), counts star imports as tie-break evidence, and keeps real per-module Gradle dependency data alive through to the final tie-break instead of discarding it when it narrows but doesn't fully disambiguate.
- **JAR-derived Java methods (no Kotlin metadata — JUnit's `Assert`, etc.) had fake arity and only one overload** — every such method was previously indexed as taking exactly one parameter regardless of its real signature, and only one overload was ever returned; `Assert.fail()`/`assertEquals(...)` and similar were effectively unresolvable. Both fixed.
- **`Outer.Nested.leaf`-shaped chains (e.g. `R.drawable.ic_account`) didn't resolve through a nested-type receiver** — plus two contributing bugs found along the way: a JAR-compiled primitive field lost its type, and same-package JAR resolution had a check that silently covered the wrong scope.
- **Qualified member calls now fall back to a supertype-walk extension when a same-named member exists with the wrong arity**, instead of returning the unusable member and stopping — `navController.navigate(route = "...")`-shaped calls were previously shadowed by an unrelated-arity overload of the same name.
- **Three further qualified-resolution gaps**: a Java getter (`getFoo()`) now retries as a Kotlin synthetic property (`foo`) when the direct lookup misses; a JAR-derived extension function now matches against its full declaration, not a partial one; the hierarchy-walk's JAR-promotion step had a budget bug that could stop it short. Recall 90.3% → 90.8%.
- **`.name`/`.ordinal` on an enum constant now resolve via go-to-definition** (previously type-inference-only); generic return-type substitution now handles a function returning the whole receiver type (`fun <T> T?.required(): T`).
- **Smart-casts and `as`/`as?` casts had several gaps**: a same-line `if (x is Y) x.foo()` smart-cast wasn't recognized; `as`/`as?` cast expressions produced no inferred type at all; `as?` is now correctly nullable (was treated identically to `as`); a same-line smart-cast no longer bleeds across a `;`-separated statement boundary; a nested class's constructor call no longer loses its qualifier and collides with an unrelated same-named leaf class.
- **Deleting a file's `package` line during a live edit no longer fails to flag it** — the "missing package declaration" diagnostic's debounced live-edit path never wired up this specific check (only a fresh file-open did); present since the diagnostic was first added, not a new regression.
- **`cargo install kmp-lsp` / publishing to crates.io was broken** — a git-commit-pinned dependency (temporary fallout from the tree-sitter 0.26 grammar migration) is rejected by crates.io's publish validation; switched back to a real published version.
- Miscellaneous: `.gitignore` cleanup (stale merge-conflict markers, `.worktrees/`); README trimmed to a lean quick-start with detail moved into `docs/`.
...
- **Grammar upgrade: tree-sitter 0.26 + tree-sitter-kotlin ABI-15** — the canonical `tree-sitter-kotlin` crate had been stuck at a ~2-year-old release on crates.io; this switches to a fork tracking upstream `main`, fixing several real parse gaps (notably the constructor-on-newline idiom) that previously left affected code un-analyzable.
- **`android.jar` is now indexed as a compiled-JAR source** — the Android SDK's platform API (`Activity`, `Bundle`, `View`, …) is auto-detected and indexed directly, rather than relying only on whatever a project's own Gradle cache happens to pull in. This alone accounted for the majority of remaining member-resolution gaps measured on a real-world benchmark corpus.
- **Kotlin's compiler-intrinsic "mapped types" resolve to their real platform declaration** — `String`, `CharSequence`, and the twelve `kotlin.collections.*` interfaces (`List`, `Set`, `Map`, `Collection`, `Iterable`, `Iterator`, and their `Mutable*` counterparts) are compiler intrinsics with **no compiled `.class` file** anywhere in kotlin-stdlib's JAR — the compiler substitutes their real JVM platform type directly into bytecode. This made a bare `String`- or `List`-typed receiver, and every extension function declared on it or an ancestor type, permanently unresolvable. Both now fall back to indexing the real `java.lang`/`java.util` declaration from the Android SDK sources bundle on demand. Measured on a real ~13k-file monorepo: member-reference recall rose from 76.4% to 86.0% and the ambiguous-candidate count roughly halved over the course of this work.
- **`kmp-lsp resolution-accuracy`** — a new CLI benchmark that classifies every identifier reference in a real codebase as resolved / ambiguous / unresolved, as a recall companion to the existing `missing-imports`/`unused-imports` precision benchmarks. Used throughout this release cycle to find and verify the fixes above and below with real, measured before/after numbers rather than guesswork.
- **Windows build ships `kmp-jar-indexer` again** — the release workflow's asset names had drifted out of sync with the `kotlin-lsp` → `kmp-lsp` rename, so Windows installs silently had no working JAR sidecar.

### Bug fixes

- **Several hand-rolled recursive tree-sitter walkers could crash the server on deeply-nested real-world code** — folding ranges, local-variable rename, and field/variable-type inference all recursed with no depth bound, matching the shape of the user's own code; each is now bounded, and the server itself now runs on a 16 MiB stack (rust-analyzer's own long-standing convention) instead of the platform default (as low as 1 MiB on Windows), covering the many CST walks whose stack use scales with source nesting.
- **A field/variable's type could resolve to the wrong thing, or not at all, when declared only on a generic superclass** — `viewModel.uiState` (declared on a generic base like `MviViewModel<S, E>`, not the concrete subclass), a receiver whose class is itself library-only (`MutableSharedFlow` extending the JAR-only `SharedFlow`), or a member reached only through Kotlin's implicit companion-object or supertype-hierarchy forwarding, could all fail to resolve — hover, go-to-definition, and inlay hints now walk the real supertype chain (including through compiled JARs) consistently everywhere a receiver's type is looked up.
- **Compiled-JAR indexing dropped a named companion object's entire member list** — Kotlin's `Foo.member` companion-forwarding idiom only ever worked for the *default*, unnamed companion (`Foo$Companion`); a *named* one (`companion object Forest : Tree()`, as Timber's entire public API — `d`, `e`, `i`, `w`, `tag`, `plant`, …  — is declared) was silently dropped, making every `Timber.d(...)`/`Timber.e(...)` call unresolvable.
- **A JAR-derived extension function's Kotlin default-valued parameters weren't marked as optional** — every compiled-JAR extension with a defaulted parameter (e.g. `kotlinx.coroutines`' `CoroutineScope.launch`) had its arity computed as fully required, so an ordinary call using the default (`scope.launch { }`) could be wrongly flagged by arg-count diagnostics or excluded from resolution.
- **Extension functions and members declared on an ancestor type, not the receiver's own class, are now found in more places** — a variable- or uppercase-qualifier-receiver call (`someString.foo()`, `Manager.requireComponent()`) now consults in-scope extensions and the class hierarchy the same way `this`/`super` calls already did; a same-package `by`/scope-function-shaped call and Compose-style member extensions without an explicit import are also covered.
- **A same-named decoy from an unrelated library could win over the real symbol during supertype-hierarchy resolution** — most commonly `com.android.internal.*` classes and Kotlin Gradle Plugin's own internal-only compiler helpers (both define decoys named things like `Activity`, `Serializable`, `CharSequence`) shadowing the real platform/stdlib type. Ambiguous hierarchy-walk results now apply a denylist and, where real per-module Gradle dependency data is available, a module-scoped tie-break.
- **Missing-import diagnostic false positives**: a bare class/constant reference used only via `Type.CONSTANT` navigation (not a type-annotation position) was invisible to the detector entirely; a stdlib-shaped star import (`import java.util.*`, `import android.*`, …) didn't suppress the flag for names it plausibly covers, even though the file demonstrably compiles — `Date`/`Calendar`/`Collections`/… were flagged as missing despite an explicit `import java.util.*`. Both fixed; the star-import fix alone cut real-corpus false positives by 45%.
- **Unused-import diagnostic false positives from two tree-sitter-kotlin parse gaps** — a body-less `fun interface` with a generic supertype, and other shapes the grammar doesn't emit a clean node for, could make a genuinely-used import look unused.
- **`when`-exhaustiveness diagnostic missed real gaps** — a subject accessed via field navigation (`userData.themeBrand`) rather than a bare identifier, and a single-line `enum class { A, B }`, both silently produced no diagnostic no matter how many branches were missing; chained smart-cast subjects are also now resolved reachability-first. Completion inside a `when` branch also now narrows correctly on equality-style branches (`Loading ->`, common for `data object` matches), not just `is` branches.
- **Stale live-edit completion cache** — a completion request that started computing against pre-edit buffer content, and finished only after a concurrent reindex had already cleared the cache, could re-populate it with the stale result; a newly-added class member could then be missing from dot-completion until the next edit.
...
- **Local variable/parameter rename now understands full Kotlin block scoping** — the local-rename fast path previously only recognized function/lambda bodies as scope boundaries, so a reference inside a nested lambda that merely *captured* an outer local (without shadowing it) incorrectly fell through to the slower cross-file path. `if`/`for`/`while`/`when`/`try`/`catch`/`finally` are now all recognized as their own scopes: a `val` declared inside one `if`/`for`/`catch` block is never conflated with an unrelated sibling block reusing the same name, and sequential re-declaration of the same name in one scope (`val x: Any = "hello"; val x = x as String`) renames each declaration and its own references independently. A named-argument label (`appState` in `SomeCall(appState = appState)`) is no longer swept into a same-named local's rename group — it names the callee's parameter, a different symbol entirely.
- **Missing-import diagnostic** — a bare class/function reference that resolves to a known symbol elsewhere in the workspace or a JAR, but isn't reachable from the file's own scope (no local declaration, explicit import, same-package visibility, non-stdlib star import, or enclosing extension/implicit-lambda receiver), is now flagged with a warning, live, as you edit — not just when a file is first opened. Kotlin's default-import packages (`kotlin.*`, `java.lang`) and their core types/functions are exempt, as is anything the JAR index hasn't materialized yet (suppressed while JAR indexing is in flight, matching call-arg diagnostics' own gate). A quick-fix ("Import '\<fully-qualified-name\>'") is offered for each flagged reference, inserting the import at the correct position — one candidate action per FQN when a bare name is ambiguous across the workspace. Verified at scale: zero false positives on nowInAndroid; single-digit false positives across a real ~12k-file/18k-total-file monorepo (Moneta).
...
- **Find-references from inside a library/JAR definition** — invoking `textDocument/references` *on the declaration* in an opened `*-sources.jar`/extracted dependency source (after go-to-definition jumped you there) now returns the workspace call sites, scoped via the JAR symbol index. The library file is never re-indexed as workspace code, and its own declaration is excluded from the results.
